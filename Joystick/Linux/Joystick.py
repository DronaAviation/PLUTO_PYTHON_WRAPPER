...
if __name__ == '__main__':
    ...
    while True:
        joystick_values = joy.read()
        # Check if the A button is pressed to arm the drone
        if joystick_values[8] == 1:  # Index 8 corresponds to the A button
            joy.arm_drone()

        # Check if the B button is pressed to disarm the drone
        if joystick_values[9] == 1:  # Index 9 corresponds to the B button
            joy.disarm_drone()
